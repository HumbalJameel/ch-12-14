...
// Question 8
const student_Names = ["Michael", "John", "Tony"];
const studentScores = [320, 230, 480];
const totalMarks = 500;

for (let i = 0; i < student_Names.length; i++) {
  const percentage = (studentScores[i] / totalMarks) * 100;
  console.log("Score of" + (student_Names[i]) + "is" + (studentScores[i]) + "Percentage:" +  percentage + "%");
}
...
// Initialize an empty array to represent our queue
const devicesQueue = [];
...
const devices = []; // Start with an empty array
...
for (let i = 0; i < manufacturers.length; i++) {dropdownHTML += "<option value='" + manufacturers[i] + "'>" + manufacturers[i] + "</option>";}
